...
def _close(server_mod):
    conn = getattr(getattr(server_mod, "meta_db", None), "conn", None)
    if conn is not None:
        conn.close()


def test_local_provider_is_default_library_provider(tmp_path, monkeypatch):
...
def test_registered_provider_handles_library_endpoints(tmp_path, monkeypatch):
    ...
    server.register_library_provider(provider)
...
def test_unknown_library_provider_returns_404(tmp_path, monkeypatch):
    server = _import_server(tmp_path, monkeypatch)
...
def test_local_library_provider_cannot_be_replaced(tmp_path, monkeypatch):
    server = _import_server(tmp_path, monkeypatch)
    provider = FakeLibraryProvider()
    provider.id = "local"
    ...
    try:
        try:
            server.register_library_provider(provider, replace=True)
